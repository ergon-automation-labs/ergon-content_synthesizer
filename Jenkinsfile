pipeline {
  ...
  environment {
    BOT_NAME = 'content_synthesizer_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-content_synthesizer"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/content_synthesizer_bot/bin/content_synthesizer_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyContentSynthesizer.Release.create()'"
          $RELEASE_BIN eval 'BotArmyContentSynthesizer.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyContentSynthesizer.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyContentSynthesizer.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_content_synthesizer/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_content_synthesizer/releases/start_erl.data)
        fi
