# Getting Started with bot_army_content_synthesizer

This guide walks you through setting up Content Synthesizer Bot for local development.
...
DATABASE_NAME=bot_army_content_synthesizer_dev
```
...
Content Synthesizer Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_content_synthesizer/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_content_synthesizer/skills/example.ex lib/bot_army_content_synthesizer/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyContentSynthesizer.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_content_synthesizer.command.classify"]
...
```elixir
# In lib/bot_army_content_synthesizer/application.ex or similar

defmodule BotArmyContentSynthesizer.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyContentSynthesizer.Skills.Classify,
      BotArmyContentSynthesizer.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_content_synthesizer/skills/my_skill.ex
...
```elixir
releases: [
  content_synthesizer_bot: [
    applications: [bot_army_content_synthesizer: :permanent]
  ]
]
```

This creates a release named `content_synthesizer_bot` that is deployed to `/opt/ergon/releases/content_synthesizer_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/content_synthesizer_bot/`
